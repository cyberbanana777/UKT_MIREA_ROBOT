...
3. Настроить IP-адрес ethernet порта на компьютере. IP-адрес должен иметь следующий вид:   `192.169.2.xxx`, xxx - любое число, кроме `15`.
4. Включить устройство, предварительно выставив его суставы (включая кисти) в нулевое положение и удерживая (можно увидеть есть включить симуляцию без подключенного УКТ), для этого необходимо перевести переключатель на корпусе устройства в рабочее положение.
...
3. Менять IP-адрес нужно, чтобы копирующее устройство и компьютер находились в одной сети. IP-адрес копирующего устройства статический и есть `192.169.2.15`. Поэтому при настройке IP-адреса компьютера нужно указать IP-адрес, который будет в этой сети и не будет принадлежать копирующему устройству. 

# Дополнительные возможности
## Отправка показаний на заданный IP-адрес
Чтобы программа-визуализатор отправляла координаты необходимо, чтобы в левом верхнем углу кнопка `Out control` была нажата. После нажатия ниже появится ещё одна кнопка и ползунок, которые контролируют масштабирование значений с датчиков. Чем больше значение - тем больше будет предел по каждой из координат. Далее будем называть `Scale` для удобства. Значения координат получаются в градусах, которые умножены на параметр `Scale`.
```
out_value = deg_value * Scale
```
- `out_value` - значение на выходе, которое мы можем получить
- `deg_value` - координата в реальных градусах
- `Scale` - параметр, который задаётся в Unity-визуализаторе
При `Scale`=1 значения, которые будут отправляться, будут в градусах.
### Пример
- `Scale` = 0.1, предел по звену №n = (-1; 1)
- `Scale` = 0.05, предел по звену №n = (-0.5; 0.5)
- `Scale` = 1, предел по звену №n = (-10; 10)
